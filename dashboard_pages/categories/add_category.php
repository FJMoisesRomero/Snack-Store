...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Normalizar el nombre de la categoría (solo para comparar)
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM categorias WHERE REPLACE(LOWER(nombre), ' ', '') = :nombre");
    ...
    if ($exists > 0) {
        session_start();
        $_SESSION['mensaje_error'] = 'La categoría ya existe.';
    ...
    $sql = "INSERT INTO categorias (nombre, created_at, updated_at, estado_activo) 
            VALUES (:nombre, NOW(), NOW(), 1)";

    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':nombre', $nombre); // Usar el nombre original sin normalizar
